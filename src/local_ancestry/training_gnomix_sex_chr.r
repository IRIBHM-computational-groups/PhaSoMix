...
OUTPUT_DIR       <- "output/local_ancestry/training"
SAMPLE_INFO_PATH <- "rawdata/1kGp/info/samples_info_CHR.txt"
CONFIG_PATH      <- "input/gnomix/"
QUERY_VCF        <- "None"
RAW_VCF          <- "rawdata/1kGp_PCAWG/rawdata/chrCHR_1kGp_phase3_PCAWG_snp_subset_1kgp_PCAWG_admixed_no_AMR.vcf.gz"
...
samples_info <- read.table("rawdata/1kGp/info/samples_info_MT.txt", header = TRUE, sep = "\t")

# NB: the column names below must match samples_info; adjust if needed.
#ancestries_pop <- unique(na.omit(samples_info$pop))
ancestries_pop <- setdiff(unique(na.omit(samples_info$pop)), "AMR")

names(list_subpop) <- ancestries_pop

list_ancestries <- c(list(POP = ancestries_pop))
...
lapply(names(list_ancestries), function(label) {
   ...
   mclapply(ALL_CHRS, function(chr) {
        ...
        samples_info_path_chr   <- gsub("CHR", chr, SAMPLE_INFO_PATH)
        samples_info_chr <- read.table(samples_info_path_chr, header = TRUE, sep = "\t")

        sample_map <- create_sample_map(ancestries, samples_info_chr,
